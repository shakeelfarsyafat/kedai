...
export const ReceiptModal: React.FC<ReceiptModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
      {/* Container Dialog */}
      <div className="bg-[#0f172a] text-slate-100 border border-slate-700 rounded-3xl w-full max-w-md max-h-[92vh] flex flex-col shadow-2xl overflow-hidden no-print">
        {/* Header Modal */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-[#1e293b]/70">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Preview Struk Pesanan</h3>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 flex justify-center bg-slate-900/60">
          {/* Printable Thermal Receipt Box */}
          <div
            id="printable-receipt-area"
            className="w-full max-w-[340px] bg-white text-black p-5 rounded-xl shadow-lg font-mono text-xs leading-tight"
          >
            {/* Store Branding */}
            <div className="text-center space-y-1 mb-4">
              ...
              <h2 className="text-base font-black tracking-wider uppercase">BREW BEAN</h2>
              <p className="text-[10px] text-gray-600">Temukan Teman Harimu</p>
            ...
            <div className="space-y-2.5 my-2">
              {order.items.map((item, idx) => (
                <div key={idx} className="space-y-0.5">
                  ...
                  {item.customization && (
                    <div className="pl-4 text-[9px] text-gray-600 space-y-0.5">
            ...
            <div className="my-3 py-1.5 px-2 text-center rounded border border-dashed border-gray-400">
              {isPaid ? (
                <div className="text-xs font-black tracking-widest text-black">
                  *** LUNAS (DIBAYAR) ***
                </div>
              ) : (
                <div className="text-xs font-black tracking-widest text-red-600">
                  *** BELUM DIBAYAR ***
                </div>
              )}
            </div>
            ...
            {/* Notes if any */}
            {order.notes && (
              <div className="text-[9px] text-gray-600 text-center italic my-1">
                Catatan: {order.notes}
              </div>
            )}
            ...
            <div className="text-center text-[9px] text-gray-500 space-y-0.5 pt-1">
              <p className="font-semibold text-gray-700">Terima kasih atas kunjungan Anda!</p>
              <p>Follow Instagram: @brewbean.coffee</p>
              <p className="text-[8px] text-gray-400 pt-1">Simpan struk ini sebagai bukti pembayaran</p>
            </div>
          </div>
        </div>

        {/* Footer Actions */}
        <div className="px-6 py-4 border-t border-slate-800 bg-[#1e293b]/70 flex items-center justify-end gap-3 no-print">
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            type="button"
            onClick={handlePrint}
            className="flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-bold text-white bg-[#007b9e] hover:bg-[#006e8d] shadow-lg shadow-cyan-950/50 transition-all active:scale-95"
          >
            <Printer className="w-4 h-4" />
